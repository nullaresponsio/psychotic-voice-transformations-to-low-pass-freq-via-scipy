...
def main() -> None:
    ...
    dX, dY = np.diff(X, axis=0), np.diff(y)
    contrib = dX * β
    all_labels = [
    ...
    for f, m, lbl in zip(features, ['^-','h-','*-','.-','s-','d-','p-'], labels):
        ax1.plot(qtrs, df[f], m, lw=2, label=lbl)
    ax1.set_title('Graph 1 – Real GDP and Component Series'); ax1.set_ylabel('Percent / Index'); ax1.grid(alpha=.3, ls='--'); ax1.legend(fontsize='x-small')
    ax2.axhline(0, ls='--', lw=1)
    bottoms = np.zeros_like(dY)
    for s, lbl in zip(contrib.T, labels):
        ax2.bar(qtrs[1:], s, bottom=bottoms, width=.65, alpha=.6, label=lbl); bottoms += s
    ax2.plot(qtrs[1:], dY, 'k.--', label='Δ GDP net')
    ax2.set_title('Graph 2 – Quarter-to-Quarter Chain-Rule Decomposition'); ax2.set_ylabel('Contribution'); ax2.legend(fontsize='x-small')
    plt.setp(ax2.get_xticklabels(), rotation=45, ha='right', fontsize='x-small'); fig.tight_layout(); fig.savefig(f'{out_base}_graphs12.png', dpi=288)

    fig3, ax3 = plt.subplots(figsize=(13.33, 5), dpi=288)
    mean_abs = np.abs(contrib).mean(axis=0); order = np.argsort(mean_abs)[::-1]
    sw_fix = [False, True, True, True, True, True, True]
    for i in order:
        ax3.barh(labels[i], mean_abs[i], alpha=.7)
    ax3.invert_yaxis(); ax3.set_xlabel('Mean |Contribution|'); ax3.set_title('Graph 3 – Ranked Drivers'); fig3.tight_layout(); fig3.savefig(f'{out_base}_graph3.png', dpi=288)

    base_products = [
        "PolicyCanvas", "SSI-Wallet++", "Fediverse Studio", "LifeLoop Coach",
        "Debt Navigator AI", "HomeGrid Orchestrator", "Climate Lab Workbench",
        "AccessLens", "MindBalance DTx", "EduQuest OER-AI", "SkillSim Arena",
        "SupplyScope", "ReliefMatch", "HealthData Coop Portal",
        "FoodPrint Optimizer", "GreenLease", "CrowdLaw Codex",
        "OpenScience Bounty Hub", "Micro-Investment Commons", "AI Ethics Auditor"
    ]

    matrix = pd.DataFrame({
        'Product': base_products,
        'Driver_Rank_Weight': list(range(1, len(base_products)+1))
    })
    matrix.to_csv(f'{out_base}_concept_products.csv', index=False)
